...
use mokuyu\image\Exception as ImageException;
use mokuyu\image\gif\Gif;
...
class Image
{
    ...
    protected function __construct( /*\SplFileInfo $file*/$file)
    {
        if (is_resource($file)) {
            $this->im = $file;
            //设置图像信息
            $this->info = [
                'width'  => imagesx($file),
                'height' => imagesy($file),
                'type'   => image_type_to_extension(IMAGETYPE_PNG, false),
